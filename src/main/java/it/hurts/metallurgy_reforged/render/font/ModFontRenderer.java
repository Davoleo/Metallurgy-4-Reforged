 ...
 * (Credit goes to Tinkers' Construct and COFHCore Devs)
 */
...
public class ModFontRenderer extends FontRenderer {
	...
	public static String getCustomFormatFromString(String text)
	{
		...
		while ((i < j - 1))
		{
			...
			// custom formatting
			else if ((int) c >= FontColor.MARKER && (int) c <= FontColor.MARKER + 0xFF)
			{
	...
	protected float renderUnicodeChar(char letter, boolean italic)
	{
		...
		if ((int) letter >= FontColor.MARKER && (int) letter <= FontColor.MARKER + 0xFF)
		{
