	...
	public static void onEntityDeath(LivingDeathEvent event)
	{
		...
		if (attacker instanceof EntityPlayer)
		{
			...
			if (player.getHeldItemMainhand().getItem() == ModMetals.CELENEGIL.getTool(EnumTools.SWORD) &&
					(player.isPotionActive(MobEffects.STRENGTH) ? player.getActivePotionEffect(MobEffects.STRENGTH).getDuration() < 8 : false ||
							!player.isPotionActive(MobEffects.SPEED) || player.getActivePotionEffect(MobEffects.SPEED).getDuration() < 8))
			{
